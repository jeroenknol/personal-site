...
void main() {
  ...
  vec2 noiseCoord = uv * vec2(4.0, 6.0);

  float tilt = 0.8 * uv.y;

  float incline = 1.5 * uv.x;
  ...
  float rotation = uv.y * uv.y * uv.y * 1.5;

  vec3 pos = vec3(
    position.x,
    position.y, 
    position.z + noise * uIntensity + tilt + 0.*incline + 0.*offset + rotation
    // position.z + noise * 0.1 + tilt + 0.*incline + 0.*offset + rotation
  );
