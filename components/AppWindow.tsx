...
export const AppWindow = ({ children }: Props) => {
  const debug = false;
  const controls = useDragControls();

  const prevX = useMotionValue<number>(0);
  const prevY = useMotionValue<number>(0);
      ...
      <div className='left-3 top-1.5 absolute flex gap-1.5'>
        ...
        <button
          ...
          onClick={() => {
            ...
            animate(prevHeight, window.innerHeight - 86.5, { duration: 0.5 });
          }}
        />
      </div>
    </motion.div>
